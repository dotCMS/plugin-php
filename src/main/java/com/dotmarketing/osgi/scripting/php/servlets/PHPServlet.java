package com.dotmarketing.osgi.scripting.php.servlets;
...
import com.dotmarketing.osgi.scripting.php.DotCMSPHPCauchoVFS;
...
public class PHPServlet extends HttpServlet {
	...
	public PHPServlet(ServiceTracker tracker){
	...
	public void init(ServletConfig config) throws ServletException {
		...
		php.setIni("allow_url_include", StringValue.create("true"));
	}
	...
	throws ServletException, IOException {
		...
		try {
			...
			env = php.createEnv(page, ws, qrequest, qresponse);
			php.setServletContext(env.getServletContext());
			...
			try {
				...
				if (prepend != null) {
					...
					if (prependPath == null)
						env.error("auto_prepend_file '" + prependPath + "' not found");
					else {
				...
				if (append != null) {
					...
					if (appendPath == null)
						env.error("auto_prepend_file '" + append + "' not found");
					else {
	...
	private Path getPath(HttpServletRequest httpReq)
	{
		QuercusHttpServletRequest req = new QuercusHttpServletRequestImpl(httpReq);
		String scriptPath = QuercusRequestAdapter.getPageServletPath(req);
		String pathInfo = QuercusRequestAdapter.getPagePathInfo(req);
		Path pwd;
		try {
			pwd = new DotCMSPHPCauchoVFS(WebAPILocator.getHostWebAPI().getCurrentHost(httpReq));
		} catch (Exception e) {
			Logger.error(PHPServlet.class,e.getMessage(),e);
			throw new DotRuntimeException(e.getMessage(), e);
		}
		Path path = pwd.lookup(pathInfo);
		return path;
	}
